...
if(isset($_GET['device_id']) && isset($_GET['password']))
{
    ...
    $Auth_query="select User_id,Email,Password,product,noofproduct from g_device join g_user using (User_id) where Device_id='$device_id' and securityKey='$securityKey'";
...
function updateDeviceStatus($deviceDetails)
{
    if(isset($deviceDetails['device_id']))
    {
        ...
        // echo $data_string;
        $data_string= $deviceDetails;
        $data_string['lastUpdated']=date('D M d Y H:i:s T O');
        unset($data_string['device_id']);
        unset($data_string['password']);
?>
    ...
    firebasePUT(url,data);
    //update the logs tohistory table
    var url="/Device/HistoryStatus/<?php echo $deviceDetails['device_id'];?>/<?php echo $data_string['lastUpdated']; ?>.json";
    var data=<?php print json_encode($data_string); ?>;
    firebasePUT(url,data);
